    ...
    <>
      <header className="bg-slate-900 border-b border-slate-800 text-white sticky top-0 z-30 shadow-xl h-20 md:h-22 px-4 md:px-6 flex items-center justify-between select-none">
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          {/* User Card - Click to open Private Account Settings */}
          {user && (
            <div className="flex items-center space-x-2 pl-2 border-l border-slate-800">
              <button
                onClick={() => setShowAccountModal(true)}
                className="flex items-center space-x-2 p-1.5 rounded-2xl hover:bg-slate-800/80 transition group text-left"
                title="Minha Conta Privada (Acessar dados pessoais)"
              >
                <div className="w-9 h-9 rounded-2xl bg-gradient-to-tr from-slate-800 to-slate-700 border border-slate-600 flex items-center justify-center text-white font-extrabold text-xs shadow-md group-hover:border-amber-500/60 transition">
                  {user.nome.charAt(0).toUpperCase()}
                </div>
                <div className="hidden xl:block text-left">
                  <div className="text-xs font-extrabold text-white leading-tight flex items-center gap-1 group-hover:text-amber-300">
                    <span>{user.nome}</span>
                  </div>
