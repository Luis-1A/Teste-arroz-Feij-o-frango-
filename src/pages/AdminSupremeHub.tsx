...
export const AdminSupremeHub: React.FC<AdminSupremeHubProps> = ({ onNavigate }) => {
  ...
  const handleExportBackup = () => {
    ...
    a.href = url;
    a.download = `bytecas_backup_completo_${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleCopyInventorySummary = () => {
    const products = localStore.getProducts();
    let txt = `📊 RESUMO DO INVENTÁRIO - BYTECAS ESTOQUE (${new Date().toLocaleDateString('pt-BR')})\n\n`;
  ...
  const sections = [
    {
      ...
      count: 10,
      items: [
        { label: 'Nome da Empresa e Identificação no Relatório', desc: 'Bytecas Vendas & Estoque Pro' },
